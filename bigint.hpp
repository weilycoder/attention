...
#ifndef BIGINT_TINY_HPP
#define BIGINT_TINY_HPP
...
struct BigIntTiny {
  ...
  BigIntTiny(long long v) {
    static char buf[21];
    sprintf(buf, "%lld", v);
    *this = buf;
  }
  BigIntTiny(unsigned long long v) {
    static char buf[21];
    sprintf(buf, "%llu", v);
    *this = buf;
  }
  void zip(int unzip) {
    if (unzip == 0) {
      for (int i = 0; i < (int)v.size(); i++)
        v[i] =
            get_pos(i * 4) + get_pos(i * 4 + 1) * 10 + get_pos(i * 4 + 2) * 100 + get_pos(i * 4 + 3) * 1000;
    } else
  ...
  std::string to_str() const { return (sign < 0 ? "-" : "") + to_astr(); }
  ...
  BigIntTiny &operator-=(const BigIntTiny &b) { return *this = *this - b; }
  BigIntTiny &operator*=(const BigIntTiny &b) { return *this = *this * b; }
  BigIntTiny &operator/=(const BigIntTiny &b) { return *this = *this / b; }
  BigIntTiny &operator%=(const BigIntTiny &b) { return *this = *this % b; }
  ...
  bool is_zero() const { return v.size() == 0 || (v.size() == 1 && v[0] == 0); }
